...
function RideDetails({
  ...
  console.log("RIDE DETAILS STATUS:", confirmedRideData?.status);
  return (
    <>
  ...
  <span
    className={`px-3 py-1 rounded text-sm text-white ${
      confirmedRideData?.status === "accepted"
        ...
        ? "bg-green-500"
        : confirmedRideData?.status === "completed"
        ? "bg-blue-500"
        : "bg-gray-500"
    }`}
  >
    Status: {confirmedRideData?.status}
  </span>
</div>
              </div>
            )}
            ...
            <div className="flex items-center gap-3 border-t-2 py-2 px-2">
              ...
              <div>
                <h1 className="text-lg font-semibold leading-6">
                  ₹ {confirmedRideData?.fare || fare[selectedVehicle]}
                </h1>
